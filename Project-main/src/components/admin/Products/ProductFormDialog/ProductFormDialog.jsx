...
import {
  ...
  getProducts,
  updateProduct,
} from "server/firebase/firestore/products";
  ...
  const handleClickSubmit = async () => {
    ...
    try {
      ...
      if (!selectedProductId) {
        ...
        showSuccessToast("Đã thêm sản phẩm thành công!");
      ...
      if (!isProductAdded) {
        showSuccessToast("Sản phẩm được cập nhật thành công!");
      }
    } catch (err) {
      setLoading(false);
      // showErrorToast(err.message);
  ...
  return (
    <React.Fragment>
      ...
      <Dialog open={open} onClose={handleClose}>
        ...
        <DialogContent>
          <DialogContentText>
            Để {Boolean(selectedProductId) ? "cập nhật thông tin" : "thêm"} sản
            phẩm, vui lòng điền các thông tin bên dưới.
          </DialogContentText>
          <TextField
            autoFocus
            margin="dense"
          ...
          <FormControl sx={{ marginBlock: 2, width: "100%" }}>
            <InputLabel id="categories">Danh mục sản phẩm</InputLabel>
            <Select
              labelId="categories"
              id="categories"
              multiple
            ...
            >
              {categoryOptions.map((option) => (
                <MenuItem key={option.value} value={option.value}>
                  <Checkbox
                    checked={formData.categories.indexOf(option.value) > -1}
                  />
          ...
          <FormControl sx={{ marginBlock: 2, width: "100%" }}>
            <InputLabel id="types">Loại sản phẩm</InputLabel>
            <Select
              labelId="types"
              id="types"
              multiple
            ...
            >
              {typeOptions?.map((option) => (
                <MenuItem key={option.value} value={option.value}>
                  <Checkbox
                    checked={formData.types.indexOf(option.value) > -1}
                  />
                  <ListItemText primary={option.label} />
                </MenuItem>
              ))}
