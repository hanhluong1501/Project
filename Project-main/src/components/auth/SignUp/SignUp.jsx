...
export default function SignUp() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!emailRegex.test(email)) {
      setToastMessage("Vui lòng nhập địa chỉ email hợp lệ");
    ...
    if (!email || !password || !confirmPassword) {
      setToastMessage("Vui lòng điền vào tất cả các trường");
      setToastSeverity("error");
      setToastOpen(true);
      return;
    }
    ...
    if (password.length < 8) {
      setToastMessage("Mật khẩu phải dài ít nhất 8 ký tự");
      setToastSeverity("error");
      setToastOpen(true);
      return;
    }
